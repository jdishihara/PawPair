...
type AuthStep = 'login' | 'signup' | 'userType' | 'ownerProfile' | 'sitterProfile' | 'complete';
...
const AuthFlow = ({ onAuthComplete }: AuthFlowProps) => {
  ...
  const handleAuth = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      if (isLogin) setCurrentStep('complete');
      else setCurrentStep('userType');
    }, 1000);
  };
  ...
  const renderInput = (placeholder: string, key: keyof UserProfile, keyboardType: any = 'default') => (
    <TextInput
      style={styles.input}
  ...
  const renderAuthScreen = () => (
    <View style={styles.centeredContainer}>
      ...
      <Text style={styles.subtext}>{isLogin ? 'Welcome back!' : 'Join the pack!'}</Text>
      ...
      <TouchableOpacity onPress={() => setIsLogin(!isLogin)}>
        <Text style={styles.link}>{isLogin ? "Don't have an account? Sign up" : 'Already have an account? Sign in'}</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  const renderUserTypeScreen = () => (
    <View style={styles.centeredContainer}>
      ...
      <TouchableOpacity style={styles.card} onPress={() => handleUserTypeSelection('owner')}>
        <Text style={styles.emoji}>🐕</Text>
        <Text style={styles.title}>Find care for my dog</Text>
        <Text style={styles.subtext}>I'm a dog owner looking for trusted sitters</Text>
      </TouchableOpacity>
      <TouchableOpacity style={styles.card} onPress={() => handleUserTypeSelection('sitter')}>
        <Text style={styles.emoji}>❤️</Text>
        <Text style={styles.title}>Care for dogs</Text>
        <Text style={styles.subtext}>I want to spend time with dogs temporarily</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  const renderOwnerProfileScreen = () => (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.button} onPress={handleProfileSubmit} disabled={loading}>
  ...
  const renderCompleteScreen = () => (
    <View style={styles.centeredContainer}>
      ...
      <TouchableOpacity style={styles.button} onPress={onAuthComplete}>
